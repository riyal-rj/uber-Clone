...
const CaptainAcceptRidePanel = (props) => {
  return (
    <div className="relative p-4 bg-white rounded-lg shadow-lg max-w-md mx-auto mt-6"> {/* Adjusted margin-top */}
      {/* Close Button */}
      <h5 className="absolute top-0 right-2 p-1 cursor-pointer" onClick={() => props.setRidePopupPanel(false)}>
        <i className="text-xl text-[#600D1C] ri-arrow-down-wide-line"></i>
      </h5>

      {/* Title */}
      <h3 className="text-lg font-bold mb-3 text-gray-800 text-center">Confirm Ride to Start</h3>
      ...
      <div className="mt-4 w-full">
        <h4 className="text-sm text-center font-semibold mb-2 text-gray-800">Enter OTP</h4>
        <div className="flex justify-between w-full gap-1">
          {/* OTP Inputs */}
          <input
            type="text"
            maxLength="1"
            className="w-8 h-8 text-center text-lg font-bold border border-[#600D1C] rounded-lg bg-[#f7f7f7] focus:ring-2 focus:ring-[#600D1C] focus:outline-none"
            placeholder="0"
          />
          <input
            type="text"
      ...
      <div className="mt-4 w-full flex flex-col gap-2">
        <Link to={'/captain-riding'} className="w-full text-sm flex justify-center bg-gradient-to-r from-green-600 to-green-800 text-white font-semibold py-2 rounded-lg shadow-lg transition-transform transform hover:scale-105">
          Confirm
        </Link>
        <button
          onClick={() => {
          ...
          className="w-full text-sm bg-gradient-to-r from-red-700 to-red-800 text-white font-semibold py-2 rounded-lg shadow-lg transition-transform transform hover:scale-105"
        >
          Cancel
        </button>
      </div>
...
export default CaptainAcceptRidePanel;
